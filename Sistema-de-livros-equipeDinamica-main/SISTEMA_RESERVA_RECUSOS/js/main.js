...
document.addEventListener('DOMContentLoaded', function() {
    console.log('🚀 Biblioteca Cesf Online - Sistema totalmente carregado');
    ...
    if (usuarioLogado) {
        ...
        // Forçar carregamento dos livros após login
        setTimeout(() => {
    ...
    // Adicionar botão admin no header se for admin
    // No main.js, atualize a função atualizarBotaoAdminHeader:
function atualizarBotaoAdminHeader() {
    const usuario = sistemaAuth.getUsuarioLogado();
    const acoesNav = document.querySelector('.acoes-nav');
    ...
    if (usuario && usuario.tipo === 'admin' && acoesNav) {
        ...
        if (!botaoAdmin) {
            botaoAdmin = document.createElement('button');
            botaoAdmin.id = 'botaoAdminHeader';
            botaoAdmin.className = 'botao botao-primario';
            botaoAdmin.innerHTML = '<i class="fas fa-cog"></i> Painel Admin';
            botaoAdmin.onclick = () => {
                // Scroll suave para área administrativa
                document.getElementById('areaAdministrativa').scrollIntoView({ 
                    behavior: 'smooth',
                    block: 'start'
                });
                
                // Abrir painel admin automaticamente
                setTimeout(() => {
            ...
            // Adicionar no início dos controles
            const primeiroBotao = acoesNav.querySelector('.botao');
            if (primeiroBotao) {
                acoesNav.insertBefore(botaoAdmin, primeiroBotao);
            } else {
                acoesNav.appendChild(botaoAdmin);
            }
        }
        ...
        botaoAdmin.style.alignItems = 'center';
        botaoAdmin.style.gap = '8px';
    }
}
    
    // Debug: Verificar se os sistemas estão carregados
    console.log('✅ Sistemas carregados:', {
        auth: typeof sistemaAuth !== 'undefined',
        livros: typeof sistemaLivros !== 'undefined',
        admin: typeof sistemaAdmin !== 'undefined',
        comentarios: typeof sistemaComentarios !== 'undefined'
    });
});