// Sistema de Mensagens Toast Melhorado - CORREÇÃO
class SistemaMensagens {
    constructor() {
        this.container = null;
        this.contador = 0;
        this.inicializarContainer();
    }

    inicializarContainer() {
        // ✅ VERIFICAR E CRIAR O CONTAINER SE NÃO EXISTIR
    ...
    mostrar(mensagem, tipo = 'info', duracao = 5000) {
        // ✅ REINICIALIZAR O CONTAINER A CADA MENSAGEM
        this.inicializarContainer();
        ...
        // ✅ CORES POR TIPO
        const cores = {
            'sucesso': { borda: '#28a745', fundo: 'linear-gradient(135deg, #28a745, #1e7e34)' },
            'erro': { borda: '#dc3545', fundo: 'linear-gradient(135deg, #dc3545, #c82333)' },
            'info': { borda: '#007bff', fundo: 'linear-gradient(135deg, #007bff, #0056b3)' },
            'aviso': { borda: '#fd7e14', fundo: 'linear-gradient(135deg, #fd7e14, #e0a800)' }
        };

        const cor = cores[tipo] || cores.info;
        mensagemElement.style.borderLeftColor = cor.borda;
        mensagemElement.style.background = cor.fundo;
        ...
        mensagemElement.innerHTML = `
            <i class="${icones[tipo] || icones.info}" style="font-size: 1.2rem;"></i>
            <span style="flex: 1; font-weight: 500; line-height: 1.4;">${mensagem}</span>
            <button class="fechar-mensagem" onclick="mensagens.fechar('${mensagemId}')" style="
        ...
        // ✅ FORÇAR REDRAW PARA GARANTIR ANIMAÇÃO
        mensagemElement.offsetHeight;
    ...
    fechar(mensagemId) {
        ...
        if (mensagemElement) {
            mensagemElement.style.animation = 'slideOutRight 0.3s ease-out';
            setTimeout(() => {
