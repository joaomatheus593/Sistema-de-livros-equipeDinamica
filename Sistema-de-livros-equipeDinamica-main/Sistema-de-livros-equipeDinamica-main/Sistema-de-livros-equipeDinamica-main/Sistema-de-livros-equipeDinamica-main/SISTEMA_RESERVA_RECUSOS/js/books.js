...
class BuscaInteligente {
    ...
    mostrarSugestoes(termo) {
        ...
        this.sugestoesContainer.innerHTML = sugestoes.map(sugestao => `
            <div class="sugestao-item" onclick="buscaInteligente.selecionarSugestao('${sugestao.termo}')">
    ...
    gerarSugestoes(termo) {
        ...
        livros.forEach(livro => {
            if (livro.titulo.toLowerCase().includes(termo)) {
                sugestoes.add(JSON.stringify({ termo: livro.titulo, tipo: 'Título' }));
            }
        });
        ...
        livros.forEach(livro => {
            if (livro.autor.toLowerCase().includes(termo)) {
                sugestoes.add(JSON.stringify({ termo: livro.autor, tipo: 'Autor' }));
            }
        });
        ...
        generos.forEach(genero => {
            ...
            if (nomeGenero.toLowerCase().includes(termo)) {
                sugestoes.add(JSON.stringify({ termo: nomeGenero, tipo: 'Gênero' }));
            }
        });
...
sistemaLivros.adicionarLivrosExemplo = function() {
    const novosLivros = [
        ...
        },
        {
            ...
            paginas: 408,
            descricao: 'Por que fazemos o que fazemos na vida e nos negócios? Um guia revolucionário para entender como os hábitos funcionam.',
            imagem: 'https://images.unsplash.com/photo-1532012197267-da84d127e765?w=400',
            estoque: 6,
            disponivel: true,
            isbn: GeradorID.gerarISBN(),
    ...
    novosLivros.forEach(livro => {
        if (!this.livros.find(l => l.titulo === livro.titulo)) {
    ...
    mensagens.sucesso(`${novosLivros.length} novos livros adicionados ao catálogo!`);
};

// Carregar livros de exemplo automaticamente se o catálogo estiver vazio
if (sistemaLivros.livros.length <= 8) {
    setTimeout(() => {
